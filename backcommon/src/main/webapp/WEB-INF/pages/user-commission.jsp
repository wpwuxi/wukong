			...
			<div class="panel panel-default">
				...
				<div id="content" class="row-fluid panel-width90"
					...
					<form class="form-horizontal" role="form" id="query"
						action="queryCommission" method="get">
						<div class="form-group">
							...
							<label for="userId" class="col-sm-1 control-label">代理人姓名</label>
							<div class="col-sm-2">
								<input type="text" class="form-control" id="userId"
									value="${userId}" name="userId" placeholder="">
							</div>

							<label for="commissionId" class="col-sm-1 control-label">代理人佣金记录号</label>
							<div class="col-sm-2">
								<input type="text" class="form-control" id="commissionId"
									value="${commissionId}" name="commissionId" placeholder="">
							</div>
						</div>
						<div class="form-group">
							<label for="time3" class="col-sm-1 control-label">佣金金额</label>
							<div class="col-sm-5">
								<input type="text" class="form-control"
							...
							<label for="backcommonId" class="col-sm-1 control-label">佣金对应的放款记录号</label>
							<div class="col-sm-2">
								<input type="text" class="form-control" id="backcommonId"
									value="${backcommonId}" name="backcommonId" placeholder="">
							</div>
					...
					<table class="table table-condensed">
						...
						<tbody id="dataBody">
							<c:forEach var="user" items="${page.list}" varStatus="status">
								<tr>
									<td>${user.commissionId}</td>
									<td>${user.backcommonId}</td>
									<td>${user.app_id}</td>
									<td>${user.amount}</td>
									<td>${user.agent_user_id}</td>
									<td>${user.name}</td>
									<td>${user.phone}</td>
									<fmt:parseDate value="${user.createdTime}" var="parsedEmpDate"
